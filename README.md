...
В этом репозитории собраны все приложения YUCT — более 60 отдельных работ, каждая со своим DOI на Figshare.
...
YUCT 是一项基本理论，主张协调优先于空间、时间、物质、能量和信息。现实并非源于粒子和场，而是源于通过共享词典和短索引（YPSDC协议）激活的系统状态之间的协调行为。
...
本仓库包含所有 YUCT 附录 —— 超过60个独立作品，每个都在Figshare上有自己的DOI。
...
本仓库中的所有材料均采用知识共享署名4.0国际许可（CC BY 4.0）进行分发，除非另有说明。
...
YUCT는 조정(코디네이션)이 공간, 시간, 물질, 에너지, 정보에 대해 우선한다는 기본 이론입니다. 현실은 입자와 장에서 발생하는 것이 아니라 공유 사전과 짧은 인덱스(YPSDC 프로토콜)를 통해 활성화되는 시스템 상태 간의 조정 행위에서 발생합니다.
...
각 부록 폴더에는 7개 언어의 자료가 포함되어 있습니다:
- 영어 (`_en.pdf`, `_en.tex`)
